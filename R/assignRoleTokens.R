assignRoleTokens <- function(stringVector,prior_=prior,
                             cores=1,thresh=0.1,thresh2=0.1,maxCol = 10,unknown=T,DT=F){
  ## exact same thing as when learning prior_! otherwise you might corrupt data
  ...
  proc <- function(stringVector){
    ...
    stringVector <- stringr::str_remove(stringVector,"'s")
    stringVector <- stringr::str_remove(stringVector,"'")
  ...
  # if string is exact match as the one in prior_ ----------------------------
  firstInd <- stringVector %in% prior_$from
  alrdyHave <- stringVector[firstInd]
  ujemanje <- match(alrdyHave,prior_$from)
  alrdyHave <- prior_[ujemanje,c(1,5:ncol(prior_))]
  ...
  if(length(stringVector>0)){ ## so we have new ones
    ...
    tmp <- lapply(which(ind), function(x){
      ...
      c(orginal[x],as.vector(as.matrix(prior_[i,5:m])) %>% na.omit() %>% as.character() %>% unique() %>% sort())
  ...
  if(length(stringVector) > 0){ ## if we have unassigned
    ...
    distMat <- stringdist::stringdistmatrix(a = stringVector,b=prior_$from,method = "jw",p=0.025,nthread = cores)
    help <- apply(distMat, 1,function(x){
      if(min(x) < thresh){
        return(prior_$org_trans[which.min(x)])
      }else{
        return(NA)
      }
    }
    ...
    ii <- na.omit(ii)
    ujemanje2 <- match(ii$org_trans,prior_$org_trans)
    ii <- cbind(from=ii$from,prior_[ujemanje2,5:ncol(prior_)],stringsAsFactors=F)
  ...
  if(exists("tmp")){ ## if there are some worth checking twice
    if(nrow(tmp)>0){
      ...
      if(length(secondGuess>0)){
        ...
        ## string distance matrix
        distMat <- stringdist::stringdistmatrix(a = second,b=prior_$from[!is.na(prior_$tokens2)],method = "jw",p=0.025,nthread = cores)
        help <- apply(distMat, 1,function(x){
          if(min(x) < thresh2){
            return(prior_$org_trans[!is.na(prior_$tokens2)][which.min(x)])
          }else{
            return(NA)
          }
        }
        ...
        ii2 <- na.omit(ii2)
        if(nrow(ii2)>0){
          ujemanje3 <- match(ii2$org_trans,prior_$org_trans[!is.na(prior_$tokens2)])
          ii2 <- cbind(from=ii2$from,prior_[!is.na(prior_$tokens2),][ujemanje3,5:ncol(prior_)],stringsAsFactors = F)
