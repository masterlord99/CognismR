...
phraseOnTopic <- function(phrase,beta){
  ...
  q2 <- ggplot(data=data.frame(topic=c2),aes(topic)) +  geom_histogram(aes(c2),bins=100) + ylab("Relative weight") +
    theme(axis.text.y = element_blank())  + ggtitle("Relative Beta (Rare Words hold more weight)")
  ...
  return(multiplot(q,a,q2,b,cols=2))
}

phraseOnTopicCombined <- function(phrase,beta,relativeBeta,k){
  fil <- data.frame(topic=1:50,beta=phrase[1:50])
  n <- max(fil$beta)
  c <- c(rep(fil$topic,fil$beta*ceiling(10000)),fil$topic)
  q <- ggplot(data=data.frame(topic=c),aes(topic)) +  geom_histogram(bins=100) + ylab("Relative Absolute weight") +
    theme(axis.text.y = element_blank()) + ggtitle("Normal Beta")
  fil2 <- data.frame(topic=1:50,beta=phrase[51:100])
  n2 <- max(fil2$beta)
  c2 <- c(rep(fil2$topic,fil2$beta*ceiling(10000)),fil2$topic)
  ...
  fil3 <- data.frame(topic=1:50,beta= (k*phrase[1:50] + (1-k)*phrase[51:100]))
  n3 <- max(fil3$beta)
  c3 <- c(rep(fil2$topic,fil3$beta*ceiling(10000)),fil3$topic)
  q3 <- ggplot(data=data.frame(topic=c3),aes(topic)) +  geom_histogram(aes(c3),bins=100) + ylab("Combined weight") +
    theme(axis.text.y = element_blank())  + ggtitle(paste0("Combined Beta with [",k,"] relative absolute and [",1-k,"] relative weights"))

  n <- nrow(fill)
  k1 <- fil$topic[order(fil$beta,decreasing = T)][1:3]
  k2 <- fil2$topic[order(fil2$beta,decreasing = T)][1:3]
  k3 <- fil3$topic[order(fil3$beta,decreasing = T)][1:3]

  a <- plotWords(beta,k1,10)
  b <- plotWords(relativeBeta,k2,10)
  c <- plotWords(beta,k3,10)
  q <- grid.arrange(q, q2,q3,a, b,c,
                    ncol=3, nrow=2, widths=c(7,7,7), heights=c(4, 10))
  return(q)
}
